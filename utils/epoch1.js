...
async function constructRewardDistributions(epochNumber) {
  ...
  // Should put into account the delegations of other people!!!

  // TODO: map delegations and voting power of each member 
  ...
  //TODO: If noone allocates to A, A's allocations have to be ignored
  ...
  console.log("ALLOCATIONS: ", allocationMap);
  ...
  const filteredAllocationMap = { ...allocationMap };
  ...
  Object.keys(filteredAllocationMap).forEach((ethAddress) => {
  ...
  console.log(filteredAllocationMap);
  ...
  console.log("RELATIVE CONTRIBUTIONS: ", relativeContributionMap);
  ...
  Object.keys(relativeContributionMap).forEach((ratedMember) => {
    Object.keys(relativeContributionMap[ratedMember]).forEach((comparedMember) => {
      ...
      Object.keys(relativeContributionMap[ratedMember][comparedMember]).forEach((rater) => {
        ...
        const raterOwnership = netOwnershipMap[rater].totalStakedDnt;
  ...
  // GO TO 50:32 to understand edgecases
  ...
  Object.keys(relativeContributionMap).forEach((ratedMember) => {
    Object.keys(relativeContributionMap[ratedMember]).forEach((comparedMember) => {
      ...
      raters.forEach((excludedRater) => {
        let average = 0;
        let total = 0;
        raters.forEach((rater) => {
          // if there's only one person rating the relationship, don't skip and use that person's opinion
          if (rater === excludedRater && raters.length > 1) {
            return;
          }
        ...
        if (!(ratedMember in excludedRelativeContributionAvg)) {
          excludedRelativeContributionAvg[ratedMember] = {};
        }
        if (!(comparedMember in excludedRelativeContributionAvg[ratedMember])) {
          excludedRelativeContributionAvg[ratedMember][comparedMember] = 0;
        }
        excludedRelativeContributionAvg[ratedMember][comparedMember] += average;
      });
    });
  });
  ...
  Object.keys(allocatedMembers).forEach((person) => {
    ...
    leftOverPie -= ((pieBites[person] + pieLeft[person]) / numberOfPeople);
    deserves[person] = eatenPie;
  });

  // Artificial Kraken's split for the people without much info
  const leftOverSplit = leftOverPie / noInfoPeople.length;
  noInfoPeople.forEach((person) => {
    deserves[person] = leftOverSplit;
  });

  // STEP 9.
  // Distribute contributor tokens according to STEP 7
  console.log(deserves)
